...
export class DirectBilling extends HttpService {
    ...
    async generateTransaction(request: DbGenerateRequest): Promise<ApiResponse<DbGenerateResponse>> {
    ...
    async getTransaction(request: DbTransactionRequest): Promise<ApiResponse<DbTransaction>> {
    ...
    async getServices(request: DbServicesListRequest): Promise<ApiResponse<DbServicesListResponse>> {
        if (!request.key) request.key = this.apiKey;
        if (!request.secret) request.secret = this.secret;
    ...
    async getTransactionLimits(request: DbTransactionLimitsRequest): Promise<ApiResponse<DbTransactionLimit[]>> {
        if (!request.key) request.key = this.apiKey;
        if (!request.secret) request.secret = this.secret;
    ...
    async getServiceCommission(request: DbServiceCommissionRequest): Promise<ApiResponse<DbCommission[]>> {
        if (!request.key) request.key = this.apiKey;
        if (!request.secret) request.secret = this.secret;
        if (!request.service_id) request.service_id = this.serviceId;
